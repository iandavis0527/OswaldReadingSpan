...
export function RadialProgress(container, cfg) {
  container.innerHTML = "";
  var nc = document.createElement("div");
  nc.style.width = "10em";
  nc.style.height = "10em";
  ...
  this.indeterminate =
    cfg.indeterminate == undefined ? false : cfg.indeterminate;
  ...
  this.fixedTextSize =
    cfg.fixedTextSize == undefined ? false : cfg.fixedTextSize;
  this.animationSpeed =
    cfg.animationSpeed == undefined
      ? 1
      : cfg.animationSpeed > 0
      ? cfg.animationSpeed
      : 1;
  this.noPercentage = cfg.noPercentage == undefined ? false : cfg.noPercentage;
  this.spin = cfg.spin == undefined ? false : cfg.spin;
  if (cfg.noInitAnimation) this.aniP = this.progress;
  else this.aniP = 0;
  ...
  this.draw = function (f) {
    if (!(f == true)) window.rp_requestAnimationFrame(this.draw);
    var c = this.canvas;
    ...
    if (
      ...
      !this.spin &&
      !this.indeterminate &&
      Math.abs(this.prevP - this.progress) < 1 &&
      this.prevW == c.width &&
      this.prevH == c.height
    )
      return;
    ...
    this.text.style.fontSize =
      (this.fixedTextSize
        ? c.clientWidth * this.fixedTextSize
        : c.clientWidth * 0.26 - this.thick) + "px";
    if (this.noAnimations) this.aniP = this.progress;
    else {
    ...
    if (this.indeterminate) {
      ...
      c.arc(
        ...
        radius,
        this.rot - Math.PI / 2,
        this.rot + this.aniP * (2 * Math.PI) - Math.PI / 2
      );
